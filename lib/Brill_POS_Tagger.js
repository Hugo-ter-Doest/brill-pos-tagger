...
Brill_POS_Tagger.prototype.parse_lexicon = function(data) {
  ...
  //logger.debug('Brill_POS_Tagger.parse_lexicon: number of lexicon entries read: ' + arrayOfLines.length);
};
...
Brill_POS_Tagger.prototype.read_lexicon = function(lexicon_file,callback) {
  ...
  fs.readFile(lexicon_file, 'utf8', function (error, data) {
    if (error) {
      ...
      logger.debug('Brill_POS_Tagger.read_lexicon: filename: ' + lexicon_file);
      if (data[0] === "{") {
        logger.debug('Brill_POS_Tagger.read_lexicon: ' + data[0]);
        that.lexicon = JSON.parse(data);
        logger.debug('Brill_POS_Tagger.read_lexicon: number of lexicon entries read (JSON): ' + Object.keys(that.lexicon).length);
        callback();
      }
      else {
        that.parse_lexicon(data);
        logger.debug('Brill_POS_Tagger.read_lexicon: number of lexicon entries read (plain text): ' + Object.keys(that.lexicon).length);
        callback();
      }
    }
  });
